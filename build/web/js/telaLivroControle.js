...
{   
    ...
    const URL_TO_FETCH='TelaLivro?acao=consultar&filtro='+filtro;
...
function ApagaAlteraLivro(acao, cod)
{   event.preventDefault(); // evita refresh da tela
    var url = "";
    switch (acao)
    {
        case "apagar":
            url = "TelaLivro?acao=apagar&cod=" + cod;
            break;
        case "alterar":
            url = "TelaLivro?acao=alterar&cod=" + cod;
            break;
    }
    ...
    fetch(url,{method:'get'/*opcional*/}).then(function(response)
    {
        response.text().then(function(result)  //response é um promisse
        {
            ...
            } else
            {
                ...
                form.paginas.value = user[1];
                form.gen.value = user[2];
                form.editora.value = user[3];
                form.nome.value = user[4];
            }
        });
    }).catch (function(err) {console.error(err);});
