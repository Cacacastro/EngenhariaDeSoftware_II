        ...
        for (Livro l : Livros) {
          res += String.format("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td>"
              ...
              + "</tr>", "" + l.getCod(), l.getTitulo(),""+l.getNumPag(), ""+l.getGen_cod(),
                        ""+l.getEdi_cod(), ""+l.getCod(), ""+l.getCod());
        }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            switch (acao.toLowerCase()) 
            {
                ...
                case "confirmar": //novo e alteração
                    ...
                    Livro liv = new Livro(cod, paginas,gen,editora,nome);
                    if (cod == 0) 
                    {   if (!ctr.salvar(liv)) erro = "Erro ao gravar o livro";}
                    else 
                    {   if (!ctr.alterar(liv)) erro = "Erro ao alterar o livro";}
                    response.getWriter().print(erro);
                    break;
            }
